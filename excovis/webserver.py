# -*- coding: utf-8 -*-
"""The Dash visualization app web server for ExCoVis."""
...
def run_temp_dir(args):
    ...
    with tempfile.TemporaryDirectory(prefix="EXCOVIS.tmp") as tmpdir:
...
def run_upload_dir(args):
    ...
    if not settings.UPLOAD_ENABLED:
        ...
        with tempfile.TemporaryDirectory(prefix="EXCOVIS.upload") as tmpdir:
...
def run_cache_dir(args):
    ...
    if settings.CACHE_TYPE == "filesystem" and not settings.CACHE_DIR:
        with tempfile.TemporaryDirectory(prefix="EXCOVIS.cache.") as tmpdir:
...
def run(args, parser):
    ...
    data_sources = []
    if os.environ.get("EXCOVIS_DATA_SOURCES"):
        data_sources += os.environ.get("EXCOVIS_DATA_SOURCES").split(";")
    ...
    if not data_sources and not args.fake_data:
        parser.error(
            "You either have to specify --data-sources or set environment variable EXCOVIS_DATA_SOURCES "
            "(or use --fake-data)"
        )
...
def setup_argparse(parser):
    ...
    parser.add_argument(
        "--host", help="Server host", default=os.environ.get("EXCOVIS_HOST", "0.0.0.0")
    )
    parser.add_argument(
        "--port", type=int, help="Server port", default=int(os.environ.get("EXCOVIS_PORT", 8050))
    )
    parser.add_argument(
        "--fake-data",
        default=False,
    ...
    parser.add_argument(
        "--cache-dir",
        default=os.environ.get("EXCOVIS_CACHE_DIR"),
        help="Path to cache directory, default is to autocreate one.",
    )
    parser.add_argument(
        "--cache-redis-url",
        default=os.environ.get("EXCOVIS_CACHE_REDIS_URL"),
        help="Redis URL to use for caching, enables Redis cache",
    )
    parser.add_argument(
        "--cache-default-timeout",
        default=os.environ.get("EXCOVIS_CACHE_DEFAULT_TIMEOUT", 600),
        help="Default timeout for cache",
    )

    parser.add_argument(
        "--upload-dir",
        default=os.environ.get("EXCOVIS_UPLOAD_DIR"),
        help="Directory for visualization uploads, default is to create temporary directory",
    )
    parser.add_argument(
        "--disable-upload",
        default=os.environ.get("EXCOVIS_UPLOAD_DISABLED", "0") not in ("", "0", "N", "n"),
    ...
    parser.add_argument(
        "--public-url-prefix",
        default=os.environ.get("EXCOVIS_URL_PREFIX", ""),
